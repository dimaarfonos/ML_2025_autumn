...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        if self._method == 'Wolfe':
            result = scipy.optimize._linesearch.scalar_search_wolfe2(
            ...
            # fallback: use Armijo if Wolfe fails
                alpha = previous_alpha if previous_alpha is not None else self.alpha_0
                while oracle.func_directional(x_k, d_k, alpha) > phi_0 + self.c1 * alpha * phi_prime_0:
                    alpha *= 0.5
                return alpha
...
def gradient_descent(oracle, x_0, tolerance=1e-5, max_iter=10000,
    ...
    grad_norm_0 = np.linalg.norm(grad_k)
    ...
    for it in range(max_iter):
        ...
        if grad_norm ** 2 <= tolerance * grad_norm_0 ** 2:
            if display:
                print(f'Итерация {it}: достигнут критерий останова')
...
def newton(oracle, x_0, tolerance=1e-5, max_iter=100,
    ...
    for it in range(max_iter):
        ...
        if display:
            print(f"iter {it:03d}: f={oracle.func(x_k):.6f}, ||grad||={np.sqrt(grad_norm_sq):.3e}, alpha={alpha:.2e}")
